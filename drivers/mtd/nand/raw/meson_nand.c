...
static const int chipsel[2] = {NAND_CE0, NAND_CE1};
...
static int meson_nfc_clk_init(struct meson_nfc *nfc)
{
	...
	value = BIT(31) | CLK_ALWAYS_ON | CLK_DIV_MASK;
	writel(value, nfc->reg_clk + SD_EMMC_CLOCK);
	...
	/* create the mux */
	mux = devm_kzalloc(nfc->dev, sizeof(*mux), GFP_KERNEL);
	if (!mux)
		return -ENOMEM;

	snprintf(clk_name, sizeof(clk_name), "%s#mux", dev_name(nfc->dev));
	init.name = clk_name;
	...
	mux->reg = nfc->reg_clk + SD_EMMC_CLOCK;
	...
	/* create the divider */
	div = devm_kzalloc(nfc->dev, sizeof(*div), GFP_KERNEL);
	if (!div)
		return -ENOMEM;

	snprintf(clk_name, sizeof(clk_name), "%s#div", dev_name(nfc->dev));
	init.name = clk_name;
	...
	div->reg = nfc->reg_clk + SD_EMMC_CLOCK;
	...
	return clk_prepare_enable(nfc->device_clk);
}
...
static int meson_nfc_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		dev_err(dev, "Failed to clk reg resource\n");
		return -EINVAL;
	}
	nfc->reg_clk = devm_ioremap_resource(dev, res);
