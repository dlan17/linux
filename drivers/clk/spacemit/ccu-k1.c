...
static struct k1_ccu_data k1_ccu_apbs_data = {
...
static const struct ccu_reset_controller_data mpmu_reset_controller_data = {
	.rst_count	= ARRAY_SIZE(mpmu_reset_data),
	.data		= mpmu_reset_data,
};

static struct k1_ccu_data k1_ccu_mpmu_data = {
...
static const struct ccu_reset_controller_data apbc_reset_controller_data = {
	.rst_count	= ARRAY_SIZE(apbc_reset_data),
	.data		= apbc_reset_data,
};

static struct k1_ccu_data k1_ccu_apbc_data = {
...
//	[RST_ISP_AHB]	= RST_DATA(APMU_ISP_CLK_RES_CTRL,	0, BIT(3)),
//	[RST_ISP_CI]	= RST_DATA(APMU_ISP_CLK_RES_CTRL,	0, BIT(16)),
	[RST_ISP_CPP]	= RST_DATA(APMU_ISP_CLK_RES_CTRL,	0, BIT(27)),
//	[RST_LCD]	= RST_DATA(APMU_LCD_CLK_RES_CTRL1,	0, BIT(4)),
//	[RST_DSI_ESC]	= RST_DATA(APMU_LCD_CLK_RES_CTRL1,	0, BIT(3)),
	[RST_V2D]	= RST_DATA(APMU_LCD_CLK_RES_CTRL1,	0, BIT(27)),
//	[RST_MIPI]	= RST_DATA(APMU_LCD_CLK_RES_CTRL1,	0, BIT(15)),
//	[RST_LCD_SPI]	= RST_DATA(APMU_LCD_SPI_CLK_RES_CTRL,	0, BIT(0)),
//	[RST_LCD_SPI_BUS] = RST_DATA(APMU_LCD_SPI_CLK_RES_CTRL,	0, BIT(4)),
//	[RST_LCD_SPI_HBUS] = RST_DATA(APMU_LCD_SPI_CLK_RES_CTRL, 0, BIT(2)),
//	[RST_LCD_MCLK] = RST_DATA(APMU_LCD_CLK_RES_CTRL2,	0, BIT(9)),
...
//	[RST_EM_AXI]	= RST_DATA(APMU_PMUA_EM_CLK_RES_CTRL,	0, BIT(0)),
//	[RST_EM]	= RST_DATA(APMU_PMUA_EM_CLK_RES_CTRL,	0, BIT(1)),
//	[RST_AUDIO_SYS] = RST_DATA(APMU_AUDIO_CLK_RES_CTRL,	0,
//				   BIT(0) | BIT(2) | BIT(3)),
...
static const struct ccu_reset_controller_data apmu_reset_controller_data = {
	.rst_count	= ARRAY_SIZE(apmu_reset_data),
	.data		= apmu_reset_data,
};

static struct k1_ccu_data k1_ccu_apmu_data = {
...
static const struct ccu_reset_controller_data rcpu_reset_controller_data = {
	.rst_count	= ARRAY_SIZE(rcpu_reset_data),
...
static const struct ccu_reset_controller_data rcpu2_reset_controller_data = {
	.rst_count	= ARRAY_SIZE(rcpu2_reset_data),
...
static const struct ccu_reset_controller_data apbc2_reset_controller_data = {
	.rst_count	= ARRAY_SIZE(apbc2_reset_data),
...
static int k1_clk_register(struct device *dev, struct regmap *regmap,
			     ...
			     const struct ccu_reset_controller_data *data)
{
	...
	rcdev->nr_resets = data->rst_count;
...
static int k1_ccu_probe(struct platform_device *pdev)
{
	...
	ret = k1_clk_register(dev, base_regmap, lock_regmap, data->clk);
	if (ret)
